...
const QRDisplay = ({ payload }: QRDisplayProps) => {
  ...
  return (
    <div className="flex flex-col items-center gap-3 w-full p-3">
      {/* QR image — fills the card width */}
      {payload.imageBase64 && (
        <img
          src={`data:image/png;base64,${payload.imageBase64}`}
          alt="Scanned QR Code"
          className="w-full max-w-[220px] h-auto rounded-lg border border-border object-contain mx-auto block"
        />
      )}
